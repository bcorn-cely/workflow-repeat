...
import { X, Send, Maximize2, Minimize2, Settings, BarChart3 } from 'lucide-react'
...
function CampaignAnalysisToolView({ 
  invocation, 
  ...
  if (invocation.state === 'approval-requested') {
    return (
      <div className="p-3 border border-border rounded mb-2 bg-card max-w-[85%]">
        <div className="text-sm mb-2 text-card-foreground">
          Approve <b>{invocation.name}</b>?
          <pre className="text-xs bg-background text-foreground p-2 rounded mt-1 overflow-x-auto border border-border">
  ...
  if (invocation.state === 'output-available') {
    return (
      <div className="max-w-[85%] text-xs bg-card border border-border rounded p-2 overflow-x-auto text-card-foreground">
        <div className="font-semibold mb-1">Campaign Analysis Started</div>
        {invocation.output?.analysisId && (
          <div className="text-muted-foreground mt-1">
            Analysis {invocation.output.analysisId} created
          </div>
        )}
      </div>
    )
  ...
  if (invocation.state === 'partial') {
    return (
      <div className="max-w-[85%] text-xs bg-card border border-border rounded p-2 text-card-foreground">
...
export function GreenlightChatbot({ id, initialMessages }: { id: string; initialMessages?: UIMessage[] }) {
    ...
    <>
      <Button
        ...
        className={cn(
          ...
          "bg-gradient-to-r from-green-500 to-emerald-600 hover:from-green-600 hover:to-emerald-700 text-white hover:scale-110",
          "before:absolute before:inset-0 before:rounded-full before:bg-green-500/20 before:animate-ping before:duration-2000",
          isOpen && "rotate-180 scale-95",
        )}
        size="icon"
      >
        {isOpen ? <X className="h-7 w-7" /> : <BarChart3 className="h-7 w-7" />}
      </Button>
      ...
      {isOpen && (
        <div ref={chatRef}>
          <Card
            className={cn(
              "pt-0 pb-6 flex flex-col shadow-2xl border-2 border-green-500/20 bg-card backdrop-blur-sm",
              isModalMode
          ...
          >
            <div className="px-6 pb-5 pt-4 border-b border-green-500/20 bg-gradient-to-r from-green-500 via-emerald-600 to-green-500 text-white rounded-t-xl" style={{ minHeight: 'fit-content' }}>
              <div className="flex items-center justify-between" style={{ contain: 'layout style' }}>
                <div className="flex items-center gap-2">
                  <BarChart3 className="h-5 w-5" />
                  <h3 className="font-semibold text-lg">Greenlight Marketing AI</h3>
                </div>
                    ...
                    >
                      ...
                      {AI_MODELS.map((model) => (
                        <DropdownMenuItem
                          ...
                          className={cn(
                            "cursor-pointer",
                            selectedModel === model.id && "bg-green-600 text-white"
                          )}
                        >
              ...
              <p className="text-sm opacity-90 mt-1">Your AI-powered marketing analytics assistant</p>
            </div>
            ...
            <div ref={messagesRef} className="flex-1 overflow-y-auto p-6 space-y-4 scroll-smooth bg-card">
              {messages.length === 0 && (
                <div className="text-center text-muted-foreground">
                  ...
                  <p className="text-base font-medium text-foreground mb-2">Welcome to Greenlight Marketing Intelligence</p>
                  <p className="text-sm text-muted-foreground mb-4">Your AI-powered marketing analytics and optimization assistant</p>
                  <div className="grid gap-2 text-left max-w-xs mx-auto">
                      <div className="flex items-start gap-2 text-sm text-foreground">
                        <span className="text-primary mt-0.5">•</span>
                        <span>Analyze campaign performance and ROI</span>
                      </div>
                      <div className="flex items-start gap-2 text-sm text-foreground">
                        <span className="text-primary mt-0.5">•</span>
                        <span>Get budget optimization recommendations</span>
                      </div>
                      <div className="flex items-start gap-2 text-sm text-foreground">
                        <span className="text-primary mt-0.5">•</span>
                        <span>Forecast campaign outcomes and CAC</span>
                      </div>
                      <div className="flex items-start gap-2 text-sm text-foreground">
                        <span className="text-primary mt-0.5">•</span>
                      ...
                      <div className="flex items-start gap-2 text-sm text-foreground">
                        <span className="text-primary mt-0.5">•</span>
                        <span>Compare channel performance and attribution</span>
                      </div>
                    </div>
                </div>
              )}
                  ...
                  {message.role === "assistant" &&
                    message.parts
                      ...
                      .map((part: any, i: number) => {
                        ...
                        // Only render if it's a tool we want to show (e.g., campaignAnalysis)
                        if (toolName === 'campaignAnalysis') {
                          return (
                            <CampaignAnalysisToolView
                              key={invocation.id}
            ...
            <form onSubmit={handleSubmit} className="px-4 pt-4 border-t border-border bg-card">
              <div className="flex space-x-2">
                <Input
                  ...
                  placeholder="Analyze campaign performance, optimize budget, forecast ROI..."
                  className="flex-1 rounded-xl bg-background border-border focus-visible:ring-primary text-foreground placeholder:text-muted-foreground"
                  autoComplete="off"
                ...
                <Button
                  ...
                  className="rounded-xl h-10 w-10 bg-primary text-primary-foreground hover:bg-primary/90"
                >
