...
function CampaignApprovalContent() {
  ...
  const handleApproval = async (approved: boolean, retryCount = 0): Promise<void> => {
    ...
    setStatus('loading');
    setMessage(approved ? 'Approving campaign analysis and recommendations...' : 'Rejecting campaign analysis...');

    try {
      const response = await fetch('/api/workflows/greenlight/campaigns/approve', {
        method: 'POST',
        headers: {
      ...
      setStatus('success');
      setMessage(approved 
        ? 'Campaign analysis and recommendations approved successfully! The workflow will continue and generate the report.' 
        : 'Campaign analysis rejected. The workflow has been notified.'
      );
    } catch (error) {
      setStatus('error');
  ...
  return (
    <div className="greenlight-theme min-h-screen flex items-center justify-center p-4 bg-gradient-to-br from-green-50/50 to-emerald-50/50 dark:from-green-950/10 dark:to-emerald-950/10">
      <Card className="p-8 max-w-md w-full shadow-lg">
        <h1 className="text-2xl font-bold mb-2">Campaign Analysis Approval</h1>
        <p className="text-muted-foreground mb-6">
          Review and approve or reject the campaign analysis and budget recommendations.
        </p>
